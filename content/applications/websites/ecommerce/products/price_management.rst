...
Site --> Homepage --> Shop`, selecting a product, and then :menuselection:`Edit --> Customize tab`
and enabling :guilabel:`Tax Indication`.
...
pricelist takes precedence over the country-based pricelist.
...
.. example::
   ...
   pricelist assigned in their contact record. This assignment tackese prescendence over the country
...
down to the :guilabel:`Shop - Products` section. There, tick the :guilabel:`Pricelist` checkbox,
then click :guilabel:`:guilabel:`Save`.
...
To utilize this feature, first navigate to :menuselection:`Website app --> Configratuion -->
Settings` and tick the :guilabel:`Prevent Sale of Zero Priced Product` checkbox, then click
:guilabel:`Save`.
...
designated as *selectable*, it allows the customer to choose between available pricelists.

.. important::
   If a pricelist is designated as *selectable*, and is not assigned to a specific website, then the
   pricelist is selectable on **all** websites.
...
- If a user's assigned pricelist differs from the selectable option, the drop-down does not appear.
...
.. note::
   If a pricelist contains a :guilabel:`Discount` price type, the striked price is visible to
   applicable customers. This is true even if the :guilabel:`Comparison Price` feature has not been
   enabled.
