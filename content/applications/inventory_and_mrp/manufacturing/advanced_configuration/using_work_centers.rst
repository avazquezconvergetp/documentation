...
*Working hours* define when a work station should operate, based on a one- or two-week schedule.
...
To configure shift-based tracking, open a work center form by going to :menuselection:`Configuration
--> Work Centers` and selecting a work center. In the :guilabel:`Working Hours` field click the
:icon:`fa-arrow-right`(:guilabel:`Internal link`) icon to edit the working hours, then click the
...
.. example::
   ...
   Starting from any exist work center, the click the :icon:`fa-arrow-right`(:guilabel:`Internal
   link`) in the :guilabel:`Working Hours` field to edit the existing working hours to match the day
   shift. Repeatedly clicking the first :icon:`fa-trash-o` :guilabel:`(trash)` icon clears out all
   the existing time fields, and each day's schedule is added using the :guilabel:`Add a line`
   button.
...
.. example::
  ...
  Shift` and `Night Shift` tags to teach of the work centers to make them more visible in list view.
   ...
   .. tip::
      ...
      top left corner to move on to the next form in the list without needing to to return to the
      list view.
